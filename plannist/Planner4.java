...
public class Planner4 {
	...
	public static void main(String[] args) {
		...
		// 构造计划时间开始时间，该范例开始时间2016-03-25 08:00:01, 结束时间2016-03-25
		// 10:00:01，当前时间2016-03-25 07:00:01
		...
		// 3.构造车辆，参数详见Bus类，此例方向1，上行路径，对应611线路。方向2，下行路径，对应612线路。
		Calendar currentTime1 = (Calendar) startTime.clone();
		currentTime1.add(Calendar.MINUTE, -55);
		Calendar currentTime2 = (Calendar) startTime.clone();
		currentTime2.add(Calendar.MINUTE, -55);
		Calendar currentTime3 = (Calendar) startTime.clone();
		currentTime3.add(Calendar.MINUTE, -25);
		Calendar currentTime4 = (Calendar) startTime.clone();
		currentTime4.add(Calendar.MINUTE, -35);
		Calendar currentTime5 = (Calendar) startTime.clone();
		currentTime5.add(Calendar.MINUTE, -45);
		Calendar currentTime6 = (Calendar) startTime.clone();
		currentTime6.add(Calendar.MINUTE, -50);
//    	public Bus(String （公车id）busId, int （最优载荷）capacity, double （单次发车成本）cost, boolean （是否在发车场，true=是，false=在途）depotStatus,Calendar （发车时间，只需在途车辆提供，在车场车辆不必提供）departureTime)
		...
		buses.add(b6);
		Bus b7 = new Bus("1777", 40, 250, false, currentTime3);
		buses.add(b7);
		Bus b8 = new Bus("1778", 40, 250, false, currentTime4);
		buses.add(b8);
		Bus b9 = new Bus("1779", 40, 250, false, currentTime5);
		buses.add(b9);
		Bus b10 = new Bus("1780", 40, 250, false, currentTime6);
		buses.add(b10);
		...
		// int[] stopIDs2=new int[trip2.size()-1];
		// for(int i=0;i+1<trip2.size();i++){
		// stopIDs2[i]=i+1;
		// }
		...
		Optimizer opt = new Optimizer(ses, buses, trip1, trip2, "611", "611",
				startTime, endTime, 10, 20, 4, 120, 0.003, 0.005, 0.3);
